$(function() {
  $('#length').append('/30');
  const words = [
    'data','components','methods','watch','scoped',
    'return','$mount','el','render','new Vue',
    'props','computed','type','required','default',
    '$emit','vm','v-bind','v-on','v-if',
    'v-else-if','v-else','v-html','v-show','v-model',
    'v-once','v-text','v-pre','v-cloak','slot',
    'v-slot','template','is','keep-alive','activated',
    'deactivated','lazy','number','trim','multiple',
    '$event','directive','inserted','componentUpdated','unbind',
    'binding',
    'arg',
    'modifiers',
    'filter',
